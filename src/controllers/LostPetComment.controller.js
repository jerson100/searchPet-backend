...
const create = async (req, res) => {
  const { _id: idUser, username } = req.user;
  const newLostPetComment = await LostPetComemntService.create(
    idUser,
    username,
    req.body
  );
  await UserActivityService.create({
    ...
    description: "Realizó un comentario",
    doc: newLostPetComment._doc._id,
    action: "c",
  });
