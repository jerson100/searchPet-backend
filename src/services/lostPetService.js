...
const create = async (
  ...
  if (foundPets.length !== petsIds.length) {
    throw new CreateLostPetException(
      "No se pudo crear la solicitud porque indicó mascotas que no existen o no le pertenecen"
    );
  }
  const imageIds = [];
  if (images) {
    ...
    for (let i = 0; i < images.length; i++) {
      try {
        ...
        throw new CreateLostPetException(
          "No se pudo crear la solicitud de perdida de su mascota porque porque no se logró subir las imágenes, vuelva a intentarlo luego."
        );
      }
    }
  }
...
const getAll = async (
  { currentLocation, minDistance, maxDistance, ...restQuery },
  pagination
) => {
  const geoJson = currentLocation
    ? (query = {}) => {
        ...
        return {
          near: {
            type: "Point",
            coordinates: [
              new Number(longitude).valueOf(),
              new Number(latitude).valueOf(),
            ],
          },
...
const getCommentsById = async (idLostPet) => {
  ...
  const comments = await LostPetComment.find(
    ...
    .populate({
      ...
      select:
        "-status -__v -updatedAt -password -district  -paternalSurname -maternalSurname -location -birthday",
    })
    .sort({
      createdAt: -1,
    });
